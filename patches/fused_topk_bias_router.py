...
# not the upstream vLLM file. Base: vLLM 0.25.1, vllm/model_executor/layers/fused_moe/router/fused_topk_bias_router.py.
# Change: `topk_softplus_sqrt()` falls back to the existing device-agnostic
# torch implementation when the router has 152 experts, for which the compiled
# CUDA kernel carries no specialization. Hash routing is preserved.
#
import functools
...
def vllm_topk_softplus_sqrt(
    ...
    # The compiled CUDA kernel has no 152-expert specialization. The existing
    # torch implementation is device-agnostic and preserves hash routing.
    if current_platform.is_xpu() or gating_output.shape[-1] == 152:
