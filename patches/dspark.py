...
# not the upstream vLLM file. Base: vLLM 0.25.1, vllm/models/deepseek_v4/nvidia/dspark.py.
# Change: `DSparkDeepseekV4Model.__init__` honours an optional
# `dspark_n_routed_experts` in the draft config, so a draft head whose routed
# expert count differs from the target's loads (152-expert target with the
# 128-expert calibrated draft head).
#
...
class DSparkDeepseekV4Model(nn.Module):
    def __init__(self, *, vllm_config: VllmConfig, prefix: str = "") -> None:
        ...
        draft_expert_count = getattr(config, "dspark_n_routed_experts", None)
        if draft_expert_count is not None:
            logger.info(
                "Using %d routed experts for DSpark instead of target's %d",
                draft_expert_count,
                config.n_routed_experts,
            )
            config.n_routed_experts = draft_expert_count
        self.config = config
        ...
        current_hf_config = current_vllm_config.model_config.hf_config
        target_expert_count = current_hf_config.n_routed_experts
        if draft_expert_count is not None:
            current_hf_config.n_routed_experts = draft_expert_count
        ...
        if draft_expert_count is not None:
            current_hf_config.n_routed_experts = target_expert_count
